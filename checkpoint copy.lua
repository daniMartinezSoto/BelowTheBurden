...
falling_spikes = {}
spike_trigger_range = 7
spike_gravity = 0.3
spike_max_fall_speed = 8


turret_defs = {
 -- clave = "x,y" en coordenadas de mapa
 ["110,13"] = {range=120, fire_rate=60, proj_speed=2},
 ["121,15"] = {range=80,  fire_rate=70, proj_speed=2},
 ["88,7"]  = {range=65, fire_rate=40, proj_speed=5},
...
function update_intro()
    ...
    elseif intro_state == "wait" then
        intro_timer += 1/30
        if intro_timer >= intro_wait_time then
            intro_state = "fadeout"
        end
    elseif intro_state == "fadeout" then
        intro_alpha -= intro_fade_speed
        if intro_alpha <= 0 then
            intro_alpha = 0
            intro_current += 1
            if intro_current > #intro_phrases then
                intro_active = false
                music(-1)  -- para la música
            else
                intro_state = "fadein"
            end
        end
...
function draw_intro()
    cls(0)
    local color = flr(intro_alpha * 7)

    local start_y = 40
    for line in all(split(intro_phrases[intro_current], "\n")) do
        print(line, 64 - #line*2, start_y, color)
        start_y += 10
    end
end
...
add(turrets,{
 ...
 cooldown = 0,
 sound_cooldown = 0,    -- NUEVO: controla cada cuánto suena
 sound_range = 50        -- NUEVO: solo suena si la esfera está dentro de este rango
})
...
function _update()
 ...
 for i=1,#puzzles do
  ...
  if all_pressed and not doors_open[i] then
   ...
   mset(door_x, door_y, 37)
   mset(door_x, door_y + 1, 52)
 ...
 for turret in all(turrets) do
  ...
  if dist <= turret.range and turret.cooldown == 0 then
    ...
    -- sonido SOLO si la torreta está visible en pantalla y cooldown de sonido listo
 ...
 for proj in all(projectiles) do
  ...
  if proj.traveled > proj.max_dist then
